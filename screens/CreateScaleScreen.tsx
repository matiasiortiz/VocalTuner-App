...
const CreateScaleScreen: React.FC = () => {
  ...
  const getNoteValue = (noteStr: string): number => {
    ...
    return (parseInt(match[2], 10) * 12) + noteIndex;
  };
  const handleSave = () => {
    if (!scaleName.trim() || sequence.length === 0) { alert("Nombre y notas requeridas."); return; }
    const rootNoteValue = getNoteValue(sequence[0].note);
    const relativeNotes: RelativeNote[] = sequence.map(s => ({ interval: getNoteValue(s.note) - rootNoteValue, duration: s.duration }));
    try {
      const saved = JSON.parse(localStorage.getItem('vocal_scales') || '[]');
      if (id) {
        const updated = saved.map((s: any) => s.id.toString() === id.toString() ? { ...s, name: scaleName, notes: sequence, relativeNotes: relativeNotes } : s);
        localStorage.setItem('vocal_scales', JSON.stringify(updated));
      } else {
        localStorage.setItem('vocal_scales', JSON.stringify([...saved, { id: Date.now().toString(), name: scaleName, notes: sequence, relativeNotes: relativeNotes, createdAt: Date.now() }]));
      }
      navigate('/scales');
    } catch (e) {}
  };
  const playSequence = async () => sequence.length > 0 && await audioService.playSequence(sequence, 120);

  const durationLabels: Record<DurationType, string> = { whole: 'Redonda', half: 'Blanca', quarter: 'Negra', eighth: 'Corchea' };
  ...
  return (
    <div className="relative flex h-full w-full flex-col md:flex-row overflow-hidden bg-[#0b0f17] shadow-2xl text-white font-display">
      <div className="flex-1 flex flex-col h-full order-1 md:order-2 overflow-hidden relative z-10 md:border-l md:border-gray-800">
        <header className="flex items-center justify-between p-4 shrink-0 bg-[#0b0f17] border-b border-white/5">
            <button type="button" onClick={() => navigate('/scales')} className="flex items-center justify-center size-12 text-slate-400 hover:text-white transition-colors active:scale-90 cursor-pointer"><span className="material-symbols-outlined text-[36px]">close</span></button>
        ...
        <main className="flex-1 overflow-y-auto no-scrollbar pb-[450px] md:pb-24 px-5 pt-6">
            ...
            <div className="mb-8"><span className="block text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-3">Duración</span><div className="flex p-1.5 bg-[#1c1f27] rounded-2xl">{(['whole', 'half', 'quarter', 'eighth'] as DurationType[]).map((dur) => (<button key={dur} type="button" onClick={() => setSelectedDuration(dur)} className={`flex-1 h-11 flex items-center justify-center rounded-xl text-[10px] font-black transition-all ${selectedDuration === dur ? 'bg-[#32394a] text-primary shadow-xl ring-1 ring-white/5' : 'text-slate-600'}`}>{durationLabels[dur]}</button>))}</div></div>
            <div className="mb-2 flex items-center justify-between relative z-[100]"><h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Secuencia Visual</h3><div className="flex items-center gap-3"><span className="text-[10px] text-primary font-black uppercase bg-primary/10 px-2 py-0.5 rounded-full">{sequence.length} Notas</span>{sequence.length > 0 && (<button type="button" onClick={() => setShowClearModal(true)} className="text-[10px] font-black text-red-500 hover:text-red-400 uppercase tracking-widest px-4 py-2 rounded-xl bg-red-500/10 hover:bg-red-500/20 transition-all">Limpiar</button>)}</div></div>
      ...
      <div className="absolute bottom-0 w-full md:static md:w-5/12 lg:w-4/12 md:bottom-auto md:h-full bg-[#151921] border-t md:border-t-0 md:border-r border-gray-800 shadow-[0_-15px_40px_rgba(0,0,0,0.6)] md:shadow-none z-30 flex flex-col order-2 md:order-1 pointer-events-auto justify-end md:justify-center">
        <div className="flex items-center justify-between px-4 py-4 gap-2 md:mb-4"><button type="button" onClick={playSequence} disabled={sequence.length === 0} className={`flex items-center gap-2 active:scale-95 transition-all shrink-0 ${sequence.length === 0 ? 'opacity-20 grayscale' : 'text-primary'}`}><span className="material-symbols-outlined fill-1 text-[36px]">play_circle</span></button><div className="flex items-center justify-center gap-1 bg-[#0b0f17] p-1 rounded-xl border border-white/5">{[3, 4, 5].map(oct => (<button key={oct} onClick={() => setCurrentOctave(oct)} className={`h-8 w-10 rounded-lg text-[10px] font-black transition-all ${currentOctave === oct ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>C{oct}</button>))}</div><div className="flex items-center gap-3 shrink-0"><button type="button" onClick={undo} disabled={sequence.length === 0} className="text-slate-500 active:text-white transition-all active:scale-90"><span className="material-symbols-outlined text-[28px]">undo</span></button><button type="button" onClick={undo} disabled={sequence.length === 0} className="text-slate-500 active:text-white transition-all active:scale-90"><span className="material-symbols-outlined text-[28px]">backspace</span></button></div></div>
        <div className="relative h-64 md:h-80 w-full select-none bg-[#101622] flex border-t border-white/5">{whiteKeys.map((note) => (<div key={note} onClick={() => addNote(note)} className="relative flex-1 bg-white border-r border-slate-300 rounded-b-2xl active:bg-slate-200 cursor-pointer shadow-inner"><span className="absolute bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-black text-slate-400 uppercase pointer-events-none">{note}</span></div>))}{blackKeysConfig.map((bk) => (<div key={`${bk.noteBase}${currentOctave}`} onClick={() => addNote(`${bk.noteBase}${currentOctave}`)} style={{ left: bk.left }} className="absolute top-0 w-[10%] h-[55%] bg-[#1a1f2b] border border-black rounded-b-xl z-10 active:bg-slate-800 shadow-2xl cursor-pointer"></div>))}</div>
      </div>
      {showClearModal && (<div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200"><div className="bg-[#161c27] rounded-3xl p-6 shadow-2xl max-w-xs w-full border border-gray-700 transform scale-100 animate-in zoom-in-95 duration-200"><div className="flex flex-col items-center text-center gap-4"><div className="size-16 rounded-full bg-orange-500/10 flex items-center justify-center text-orange-500 mb-1 border border-orange-500/20"><span className="material-symbols-outlined text-3xl">cleaning_services</span></div><div><h3 className="text-xl font-bold mb-2 text-white">¿Limpiar Notas?</h3><p className="text-sm text-gray-400">¿Deseas borrar la secuencia?</p></div><div className="flex gap-3 w-full mt-3"><button onClick={() => setShowClearModal(false)} className="flex-1 h-12 rounded-xl font-bold text-gray-300 bg-gray-800 transition-colors">Cancelar</button><button onClick={confirmClear} className="flex-1 h-12 rounded-xl font-bold text-white bg-orange-500 hover:bg-orange-600 shadow-lg transition-all active:scale-95">Borrar</button></div></div></div></div>)}
    </div>
  );
};
