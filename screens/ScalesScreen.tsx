...
const ScalesScreen: React.FC = () => {
  ...
  return (
    <div className="bg-[#0b0f17] text-white min-h-screen pb-32 w-full relative font-display">
      ...
      {/* Controles de Configuración Global - Más anchos para escritorio */}
        ...
        <div className="flex flex-col lg:flex-row gap-6 max-w-screen-2xl mx-auto items-center">
          <div className="flex flex-col gap-1 w-full lg:w-1/4">
             ...
             <div className="flex gap-3">
                <span className="text-xs bg-primary/20 text-primary px-3 py-1.5 rounded-lg font-black border border-primary/30 uppercase tracking-tighter">{selectedNote} {startOctave}-{endOctave} Oct</span>
                <span className="text-xs bg-gray-800 text-gray-300 px-3 py-1.5 rounded-lg font-black border border-gray-700 uppercase tracking-tighter">{bpm} BPM</span>
             </div>
          </div>
          ...
          <div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-4 gap-3 w-full lg:flex-1">
             <div className="relative">
                <select 
                  ...
                  className="w-full h-11 bg-[#1c2333] border border-gray-700 rounded-xl text-sm font-bold px-4 appearance-none outline-none focus:border-primary transition-all"
                >
                  {NOTES.map(n => <option key={n} value={n}>{n}</option>)}
                </select>
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-gray-500"><span className="material-symbols-outlined text-sm">expand_more</span></div>
             </div>
        ...
        {customScales.length === 0 ? (
          <div className="py-32 flex flex-col items-center justify-center text-center bg-white/[0.02] rounded-[40px] border-2 border-dashed border-gray-800">
            ...
            <button 
              ...
              className="h-14 px-10 bg-primary text-white text-sm font-black rounded-2xl shadow-glow uppercase tracking-widest active:scale-95 transition-all hover:scale-105"
            >
              Crear Nueva Escala
            </button>
          </div>
              ...
              >
                <div className="flex flex-col gap-5 relative z-10">
                  <div 
                    ...
                    className="flex items-center gap-4 cursor-pointer group/info"
                  >
                    <div className={`size-14 rounded-2xl flex items-center justify-center transition-all shrink-0 ${playingId === scale.id ? 'bg-red-500 text-white shadow-xl shadow-red-500/30 rotate-3' : 'bg-primary/10 text-primary group-hover/info:bg-primary/20'}`}>
                      <span className="material-symbols-outlined text-3xl pointer-events-none">
                  ...
                  <div className="flex items-center gap-3 pt-2 border-t border-white/[0.05]">
                    <button 
                      ...
                      className="flex-1 h-11 rounded-xl bg-blue-500/10 text-blue-500 hover:bg-blue-500 hover:text-white flex items-center justify-center gap-2 transition-all active:scale-90 font-bold text-sm border border-blue-500/20"
                    >
                    ...
                    <button 
                      ...
                      className="size-11 rounded-xl bg-red-500/10 text-red-500 hover:bg-red-600 hover:text-white flex items-center justify-center transition-all active:scale-90 border border-red-500/20"
                    >
            ...
            <div className="flex flex-col items-center text-center gap-6">
              ...
              <div className="flex flex-col gap-3 w-full mt-2">
                <button 
                  ...
                  className="w-full h-14 rounded-2xl font-black text-white bg-red-500 hover:bg-red-600 shadow-xl shadow-red-500/30 transition-all active:scale-95 uppercase tracking-widest text-xs"
                >
                  Confirmar Eliminación
                </button>
                <button 
                  onClick={() => setScaleToDelete(null)}
                  className="w-full h-14 rounded-2xl font-bold text-gray-400 hover:text-white transition-colors"
                >
                  Cancelar
                </button>
              </div>
